...
class EmpenhoManagerContratos(QWidget):
    ...
    def consultar_empenhos(self):
        ...
        url = f"https://contratos.comprasnet.gov.br/api/contrato/{self.id}/empenhos"
        print(f"Consultando API: {url}")
        
        try:
            response = requests.get(url, timeout=10)
            response.raise_for_status()
            empenhos = response.json()  # Supõe que o retorno é um JSON
            print(empenhos)  # Print no console
            self.popular_detalhes(empenhos)
